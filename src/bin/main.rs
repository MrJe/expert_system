...
fn expert_system(file: File) {
	let solved_facts = solver(&file).unwrap();
	match print_solved_to_file(OUTPUT_FILE, solved_facts) {
			Ok(())	=> println!("Expert system worked as expected, open {} to see the results!", OUTPUT_FILE),
			Err(error)	=> println!("Oops, something went wrong, shutting program down.\n Error: {}", error),
	}
}
